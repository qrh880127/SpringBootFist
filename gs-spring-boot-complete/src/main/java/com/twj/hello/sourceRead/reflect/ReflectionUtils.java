package com.twj.hello.sourceRead.reflect;
...
public abstract class ReflectionUtils {
	...
	/**
	 * 被CGLIB重命名的方法的方法的前缀，用来过滤被CGLIB重命名的方法
	 * 
	...
	/**
	 * 使给定类的所有方法都执行回调mc的doWith方法，不含方法过滤器
	 * 		注意：如果父类/子类里含有相同的方法，则doWith()会调用多次，除非添加方法过滤器MethodFilter
	 * 		使用场景1:使类的所有方法都可访问
	 * 			参加：MessageListenerAdapter
	 * 			ReflectionUtils.doWithMethods(c, new MethodCallback() {
					public void doWith(Method method) throws IllegalArgumentException, IllegalAccessException {
						ReflectionUtils.makeAccessible(method);	//使所有方法都可访问
						methods.add(method);
					}
				}, new MostSpecificMethodFilter(methodName, c));	//过滤方法
	 * 		
	 * 
	 * Perform the given callback operation on all matching methods of the given
